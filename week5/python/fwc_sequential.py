...
def read_world_model(filename):
    ...
    try:
        ...
        print("Error: h5py not installed. HDF5 reading not available.", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(f"Error reading world model: {e}", file=sys.stderr)
        sys.exit(1)
...
def write_hdf5(world, filename, iteration):
    ...
    try:
        ...
        print(f"Error writing to HDF5: {e}", file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage='./fwc_sequential.py --iter <number of iterations> --model <input model> --out <name of output file>'
    )
