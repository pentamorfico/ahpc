/*
 * OpenMP Strategy 3: Conservative Task-Based Parallelization
 * 
 * This strategy uses a more conservative approach to task parallelism,
 * ensuring identical computation order and results while still
 * demonstrating task-based concepts.
 */
...
DoubleVector propagator(DoubleVector wave, DoubleVector density, DoubleVector velocity) {
    ...
    // Using tasks only for independent operations that don't affect computation order
    // ========================================
    
    // Phase 1: Initial computations - can be done with tasks
    #pragma omp parallel
    {
        #pragma omp single
        {
            // Task 1: Impedance computation
            #pragma omp task
            {
            ...
            // Task 2: Filter preparation (independent of impedance)
            #pragma omp task
            {
                #pragma omp parallel for schedule(static)
            ...
            // Task 3: Wave preparation (independent)
            #pragma omp task
            {
                #pragma omp parallel for schedule(static)
    ...
    // Reflection coefficients depend on impedance
    #pragma omp parallel for schedule(static)
    ...
    // Phase 3: Final processing with tasks for independent operations
    #pragma omp parallel
    {
        #pragma omp single
        {
            // Task 4: Apply filter and setup first part of Upad
            #pragma omp task
            {
                #pragma omp parallel for schedule(static)
                for (long i=0; i < nfreq+1; i++) {
            ...
            // Wait for first part to complete before mirroring
            #pragma omp taskwait
            
            // Task 5: Upad mirroring
            #pragma omp task
            {
                #pragma omp parallel for schedule(static)
...
int main(int argc, char* argv[]) {
    std::cout << "=== OpenMP Strategy 3: Conservative Task-Based Parallelization ===" << std::endl;
