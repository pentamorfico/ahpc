/*
 * OpenMP Strategy 4: SIMD + Parallel Loops Optimization
 * 
 * This strategy combines traditional parallel for loops with
 * SIMD directives to exploit both thread-level and instruction-level
 * parallelism.
 * 
 * Expected benefits:
 * - Better vectorization of inner loops
 * - Combines thread and SIMD parallelism
 * - More explicit vector optimization
 * 
 * Expected drawbacks:
 * - SIMD may not always be beneficial
...
DoubleVector propagator(DoubleVector wave, DoubleVector density, DoubleVector velocity) {
    ...
    // Wave spectral processing with SIMD and reduction
    #pragma omp parallel for simd reduction(+:mean_wave) schedule(static)
    ...
    // Upgoing waves computation - more complex, use regular parallel for
    // as SIMD may not benefit complex inner loops
...
int main(int argc, char* argv[]) {
    std::cout << "=== OpenMP Strategy 4: SIMD + Parallel Loops Optimization ===" << std::endl;
