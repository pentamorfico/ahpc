...
- **Problem Scaling**: NFREQ = 65,536 $\times$ threads (weak scaling protocol)
...
**Data Volume**: 210 individual measurements (6 strategies $\times$ 7 thread counts $\times$ 5 repetitions)
...
**Key Observations**:
- **Ideal weak scaling**: Horizontal dashed line shows ideal behavior (constant normalized time = 1.0)
- **Dramatic deviation**: All strategies show severe departure from ideal weak scaling
- **Consistent degradation**: All strategies show similar poor scaling behavior with ~10x time increase
- **Error bars**: Small variance across repetitions confirms reliable measurements  
- **Linear degradation**: Near-linear increase in normalized time reveals memory bandwidth limitation
- **Strategy independence**: OpenMP implementation has minimal impact on fundamental scaling limits
...
- **Ideal behavior**: Horizontal dashed line represents perfect weak scaling (constant time)
- **O(N log N) consideration**: FFT cost scales as NFREQ $\times$ log\_2(NFREQ)
- **Expected weak scaling**: For NFREQ = 65,536 $\times$ P, ideal cost scaling is P $\times$ (16 + log\_2(P))
- **Observed behavior**: Linear 10x increase indicates memory bandwidth saturation dominates
- **Theoretical vs. actual**: Should scale as ~P $\times$ (16 + log\_2(P)) but observed as ~10$\times$P
...
**Combined Analysis**:
- **Ideal weak scaling**: Horizontal dashed line shows target behavior (normalized time = 1.0)
...
- **Weak scaling failure**: 10$\times$ runtime increase for 64$\times$ problem size increase demonstrates severe scalability limits
...
1. **Theoretical Model**: For FFT with input size N = NFREQ = 65,536 $\times$ P:
   ```
   T_FFT(P) = C\_1 $\times$ P $\times$ log\_2(65,536 $\times$ P) = C\_1 $\times$ P $\times$ (16 + log\_2(P))
   ```

2. **Expected Weak Scaling**: If perfectly parallel:
   ```
   T_FFT(P) / T_FFT(1) = (16 + log\_2(P)) / 16 $\approx$ 1 + log\_2(P)/16
   ```

3. **Observed Scaling**: Linear increase T_FFT(P) $\approx$ 10 $\times$ T_FFT(1) $\times$ P

4. **Parallel Fraction Computation**: Using modified Amdahl's law:
   ```
   Efficiency = 1 / (f_serial + (1 - f_serial) $\times$ overhead_factor)
   ```
...
- **Outlier removal**: Applied 3$\sigma$ filtering to remove computational anomalies
