...
class Vec3:
    def __init__(self, x: float, y: float, z: float): # initialization of vector
        self.x = x
        self.y = y
        self.z = z

    def mag2(self) -> float:  # squared size of vector (slightly faster)
        return self.x * self.x + self.y * self.y + self.z * self.z

    def mag(self) -> float:  # size of vector
        return np.sqrt(self.mag2())

    def __sub__(self, other: "Vec3") -> "Vec3":  # subtraction of two vectors
        return Vec3(self.x - other.x, self.y - other.y, self.z - other.z)

    def __add__(self, other: "Vec3") -> "Vec3":  # addition of two vectors
        return Vec3(self.x + other.x, self.y + other.y, self.z + other.z)

    def __mul__(self, scalar: float) -> "Vec3":  # multiplication of vector by scalar (vec x scalar)
        return Vec3(scalar * self.x, scalar * self.y, scalar * self.z)

    def __rmul__(self, scalar: float) -> "Vec3":  # multiplication of scalar by vector (scalar x vec)
        return self.__mul__(scalar)

    def __truediv__(self, scalar: float) -> "Vec3":  # division of vector by scalar
        return Vec3(self.x / scalar, self.y / scalar, self.z / scalar)

    def __iadd__(self, other: "Vec3") -> "Vec3":  # add and assign to vector
        self.x += other.x; self.y += other.y; self.z += other.z
        return self

    def __isub__(self, other: "Vec3") -> "Vec3":  # subtract and assign to vector
        self.x -= other.x; self.y -= other.y; self.z -= other.z
        return self

    def __imul__(self, scalar: float) -> "Vec3":  # multiply and assign to vector
        self.x *= scalar; self.y *= scalar; self.z *= scalar
        return self

    def __itruediv__(self, scalar: float) -> "Vec3":  # divide and assign to vector
        self.x /= scalar; self.y /= scalar; self.z /= scalar
        return self
...
def UpdateKDK(sysobj: System, sc: Sim_Configuration):
    for molecule in sysobj.molecules:
        for atom in molecule.atoms:
            ...
            atom.f = Vec3(0.0, 0.0, 0.0)  # set the forces zero to prepare for next potential calculation
...
def MakeWater(N_molecules: int) -> System:
    ...
    for i in range(N_molecules):
        ...
        Oatom.p = Vec3(P0.x, P0.y, P0.z)
        Hatom1.p = Vec3(P0.x + L0 * np.sin(angle / 2), P0.y + L0 * np.cos(angle / 2), P0.z)
        Hatom2.p = Vec3(P0.x - L0 * np.sin(angle / 2), P0.y + L0 * np.cos(angle / 2), P0.z)
...
def WriteOutput(sysobj: System, file):
    ...
    for molecule in sysobj.molecules:
        for atom in molecule.atoms:
            ...
            file.write(
                f"{sysobj.time:.6g} {atom.name} {atom.p.x:.6g} {atom.p.y:.6g} {atom.p.z:.6g}\n"
            )
